    ...
    if msg.is_multipart():
        for part in msg.walk():
            if part.get_content_type() == "text/plain":
                # text/plain multipart fix (original error area)
                text_content += part.get_payload(decode=True).decode(
                    part.get_content_charset("utf-8"), errors="ignore"
                )
            elif part.get_content_type() == "text/html":
                # --- FIX 1: Add errors='ignore' to text/html decoding ---
        ...
        if msg.get_content_type() == "text/plain":
            # --- FIX 2: Add errors='ignore' to singlepart text/plain decoding ---
            text_content = msg.get_payload(decode=True).decode(
                msg.get_content_charset("utf-8"), errors="ignore"
            )
        elif msg.get_content_type() == "text/html":
            # --- FIX 3: Add errors='ignore' to singlepart text/html decoding ---
...
def search_and_process_emails(
    ...
    # Note: Using M.search(None, search_criteria) without separating the arguments 
    # might cause issues if search_criteria contains spaces, but we leave it as is
    # to match the original structure, focusing on the decoding fix.
